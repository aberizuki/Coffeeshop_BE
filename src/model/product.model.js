...
const productModel = {
  ...
  getDetail: (id) => {
    return new Promise((resolve, reject) => {
      db.query(
        `SELECT product.id, product.title, product.price, product.category,
        json_agg(row_to_json(product_images)) images FROM product INNER JOIN product_images ON product.id=product_images.id_product WHERE product.id='${id}' GROUP BY product.id`,
        (err, result) => {
          if (err) {
            return reject(err.message);
          } else {
